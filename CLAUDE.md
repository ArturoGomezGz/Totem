...
- **`deploy/db/schema.sql`** — schema real ejecutado contra TimescaleDB. Es la fuente de verdad de la DB; no hay Alembic ni otra herramienta de migraciones todavía — los cambios de esquema se editan directamente ahí.
...
**Antes de asumir el estado de una parte del sistema, revisa el código en `server/`, `frontend/`, `firmware/`, `simulator/` y `deploy/db/schema.sql` en vez de guiarte solo por este archivo o por `docs/` — la documentación puede quedar desactualizada respecto a la implementación real.**
