...
| `co2` | FLOAT | Nullable — Senseair S8 (NDIR) por UART/Modbus. **ppm ya calibrados** por el sensor (no conteo crudo del ADC) — fase de prueba, solo monitoreo |
...
**Decisión — 16 jul 2026.** Se reintroduce `co2` en `readings` (migración `e5f9c2a7d1b3`) para un Senseair S8 (NDIR) agregado a `firmware/monitorio` 1.3.4, **solo monitoreo**. Esto **no revierte** la decisión del 10 jul de sacar el CO₂ de la *decisión de riego* (ver `capa1/totem-principal/sistema-decision/modulo-decision.md`): el riego sigue siendo por VPD y **no** se reintroducen `crop_profiles.co2_min/co2_max`. El S8 habla Modbus por UART y entrega ppm ya calibrados (a diferencia del conteo crudo de MQ-4/Grove), así que la columna guarda ppm directo.
