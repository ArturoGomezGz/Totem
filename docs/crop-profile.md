...
- 🔴 **Pendiente:** la función exacta que mapea Pn → duración del ciclo (función lineal, tabla de rangos discretos, u otra). Ver `docs/irrigation-module.md`.
...
3. El backend encola update_profile como comando pendiente para ese ESP32
4. El ESP32, en su próximo ciclo de polling, descarga el perfil actualizado (FR-09)
...
- **Perfil por defecto (factory state)** — ¿qué hace el ESP32 si arranca por primera vez sin haber recibido nunca un perfil? ¿Valores conservadores hardcodeados, o espera conexión antes de operar? Ver `docs/architecture.md` — decisiones pendientes.
...
- `docs/irrigation-module.md` — el umbral de Pn y la función de duración dependen del modelo ML
- `docs/architecture.md` — el perfil vive en la DB de Capa 2 y se cachea en flash del ESP32
- `docs/schema.md` — tabla `crop_profiles`
