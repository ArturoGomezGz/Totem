...
**`firmware/` y `server/` son independientes por diseño.** La Capa 1 (ESP32) opera sin la Capa 2 — si el server cae, el riego sigue funcionando. Ninguno depende del código del otro; solo comparten el contrato de API (a documentar en `docs/api-contract.md` una vez definido).
...
**`ml/` es independiente del server** porque el flujo de trabajo de entrenamiento (notebooks, datasets, experimentos) es distinto al del servidor. Los modelos entrenados se exportan al firmware (`.tflite` para inferencia en dispositivo) o al server según donde corra la inferencia — decisión pendiente de tomar.
